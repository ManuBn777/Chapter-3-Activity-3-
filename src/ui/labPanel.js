...
export function createLabPanel({ params, onReset, onPreset, onBeat, onModeChange, onPauseChange }) {
  ...
  const state = {
    ...
    beatDecay: params.beatDecay.value
  };
  ...
  // INSTRUMENTOS Y ESTADOS
  const instruments = document.createElement('div');
  instruments.className = 'group';
  instruments.innerHTML = '<h2>Forces / Instrument</h2>';
  panel.append(instruments);
  ...
  refreshers.push(rangeRow(instruments, 'beatDecay (duración pulso)', state, 'beatDecay', 0.3, 4, 0.05, (v) => params.beatDecay.value = v, () => params.beatDecay.value));
